...
app.get('/Tx', function (req, res) {
  const  type = "SELECT";
  const sql = 'SELECT * FROM Reg_Tx WHERE ClientID =  ' + mysql.escape(req.body.ClientID) + ' AND txID = ' + req.body.txID;
  const data = [type, sql];
...
app.post('/Add_tx', function (req, res) {
  ...
  const sql = 'INSERT INTO Reg_Tx (ClientID, txID , NID,  Libelle, Date_operation, amount) VALUES ?'; 
...
app.get('/Account', function (req, res) {
  const  type = "SELECT";
  const sql = 'SELECT * FROM Reg_Accounts WHERE ClientID =  ' + mysql.escape(req.body.ClientID);
  const data = [type, sql];
...
app.post('/account', function (req, res) {
  ...
  const sql = 'INSERT INTO Reg_Accounts (ClientID, NID , Name,  Lastname, Job, Date_operation) VALUES ?'; 
  const data = [type, sql, values];
